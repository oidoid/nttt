...
import { cellParseDSL, cellToString } from './cell.ts'
 ...
 */
export function Board(size = 3): Board {
 ...
 */
export function boardParseDSL(dsl: string): readonly Cell[][] {
  const cells = dsl.replace(/\s/g, '').split('').map(cellParseDSL)
...
const aggregateState: Readonly<
  Record<BoardState, Readonly<Record<BoardState, BoardState>>>
> = {
  x: { x: 'x', o: 'xo', xo: 'xo', '?': '?' },
  o: { x: 'xo', o: 'o', xo: 'xo', '?': '?' },
  xo: { x: 'xo', o: 'xo', xo: 'xo', '?': '?' },
 ...
 * the whole `Board`.
 */
export function boardGetState(self: Readonly<Board>): BoardState {
  ...
  // boards).
  let boardState: BoardState = self[0]![0]!

  // The aggregated state of each column.
  const columnState: BoardState[] = [...self[0]!]

  // Check for horizontal lines and tally columns.
  for (let y = 0; y < size; y++) {
    let rowState: BoardState = self[y]![0]!
  ...
  // Diagonal line test.
  let forward: BoardState = self[0]![0]!
  let backward: BoardState = self[0]![size - 1]!
 ...
 *  `y`-coordinates.
 */
export function boardMark(self: Board, cell: Cell, x: number, y: number): void {
  const size = self.length
 ...
 */
export function boardToString(self: readonly (readonly Cell[])[]): string {
  const size = self.length
  if (!size) return ''
  if (size === 1) return cellToString[self[0]![0]!]
  ...
  const middle = self
    .map((row) => `${row.map((cell) => cellToString[cell]).join('│')}\n`)
    .join(divider)
